...
{
    /**
     * Compute a new run using score fusion combMNZ.
     *
     * @param run Run to merge: [run index][rank].
     * @return A new run.
     */
    public static RunEntry[] combMNZ(RunEntry[][] run)
    {
        /*
    ...
    /**
     * Compute a new run using score fusion rCombMNZ.
     *
     * @param run Run to merge: [run index][rank].
     * @return A new run.
     */
    public static RunEntry[] rCombMNZ(RunEntry[][] run)
    {
        /*
        ...
        for (int i = 0; i < run.length; i++)
        {
            for (int j = 0; j < run[i].length; j++)
            {
                // Get current entry property
                String currKey = run[i][j].id;
                double currScore = run[0].length-1-j;
    ...
    /**
     * Compute a new run using score Borda fuse.
     *
     * @param run Run to merge: [run index][rank].
     * @return A new run.
     */
    public static RunEntry[] bordaFuse(RunEntry[][] run)
    {
        /*
         * For every ranked document, assign r - # doc points, then
         * accumulate the sum of all the points and consider it as the
    ...
    public static RunEntry[] condorcetFuse(RunEntry[][] run)
    {
        /*
        ...
        finalRun = Util.sortRunMajRunoff(finalRun, run);
